...
class ControllerCheckoutConfirm extends Controller {
	public function index() {
		...
		if (!$redirect) {
			...
			WriteToLog("AAAAA " . ReturnVarDump($order_data) . "\n");
			...
			// 2016-01-13 / добавлено для совместимости с методом quick
			return 1;
		} else {
	...
	// 2016-01-13
	// принудительно зафигачить корзину
	public function quick() {
		$phone = (isset($this->request->post['phone']) && $this->request->post['phone'] != '' ? $this->request->post['phone'] : 
			(isset($this->request->post['single_phone']) ? $this->request->post['single_phone'] : '')
			);
		$fio = (isset($this->request->post['fio']) && $this->request->post['fio'] != '' ? $this->request->post['fio'] : 
			(isset($this->request->post['single_phone']) ? $this->request->post['single_phone'] : '')
			);
		$email = (isset($this->request->post['email']) && $this->request->post['email'] != '' ? $this->request->post['email'] : ' ');
		$city = (isset($this->request->post['city']) ? $this->request->post['city'] : '');
		$delivery = (isset($this->request->post['delivery']) ? $this->request->post['delivery'] : '');
...
//WriteToLog('11111 ' . ReturnVarDump($_POST) . "\n");
		...
		$this->session->data['guest']['firstname'] = $fio;
		$this->session->data['guest']['telephone'] = $phone;
		$this->session->data['shipping_address']['city'] = $city;
		$this->session->data['comment'] = $comment;
		$this->session->data['payment_method']['title'] = $payment;
		$this->session->data['shipping_method']['title'] = $delivery;
		...
		$this->session->data['shipping_address']['zone_code'] = 0;
		...
		$this->session->data['payment_address']['address_format'] = $this->session->data['shipping_address']['address_format'];
		...
		$this->session->data['payment_address']['country'] = $this->session->data['shipping_address']['country'];
		$this->session->data['payment_address']['country_id'] = $this->session->data['shipping_address']['country_id'];
		$this->session->data['payment_address']['custom_field'] = $this->session->data['shipping_address']['custom_field'];
		$this->session->data['payment_address']['firstname'] = $this->session->data['shipping_address']['firstname'];
		$this->session->data['payment_address']['iso_code_2'] = $this->session->data['shipping_address']['iso_code_2'];
		$this->session->data['payment_address']['iso_code_3'] = $this->session->data['shipping_address']['iso_code_3'];
		...
		$this->session->data['payment_address']['zone'] = $this->session->data['shipping_address']['zone'];
		$this->session->data['payment_address']['zone_code'] = $this->session->data['shipping_address']['zone_code'];
		...
		if ($actionIndex) {
			...
			// $response['error'] = 'Что-то пошло не так. Пожалуйста, сообщите об этом консультанту магазина по телефону';
			// $response['error'] = $this_index;
		}
	}
}